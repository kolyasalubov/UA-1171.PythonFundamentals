...
while run:
    ...
    if keys[pygame.K_RIGHT] and not COORD_X >= WIDTH_DISPLAY - (WIDTH_RECTANGLE + DELTA_STEP):
        COORD_X = COORD_X+DELTA_STEP
    if keys[pygame.K_UP] and COORD_Y > DELTA_STEP:
        COORD_Y = COORD_Y-DELTA_STEP
    if keys[pygame.K_DOWN] and not COORD_Y >= HEIGHT_DISPLAY - (HEIGHT_RECTANGLE + DELTA_STEP):
        COORD_Y = COORD_Y+DELTA_STEP
